...
var TRACE_DIR = path2.join(ARTIFACT_DIR, TRACE_SUBDIR);
var DEFAULT_AGENT_CONFIG = path2.join("agent", "agent.ts");
...
function traceDir(artifactDir = ARTIFACT_DIR) {
  return path2.join(artifactDir, TRACE_SUBDIR);
}
...
// src/core/load-project.ts
var typescriptLoaderRegistered = false;
async function loadAgentProject(cwd) {
  return (await loadAgentProjectContext(cwd)).project;
}
async function loadAgentProjectContext(cwd) {
  const tsConfigPath = path3.resolve(cwd, DEFAULT_AGENT_CONFIG);
  const yamlPath = path3.resolve(cwd, OPENPOND_MANIFEST);
  ...
  if (hasTypescriptConfig) {
    ...
    const mode = project.manifestMode === "extends-openpond-yaml" ? "extends-openpond-yaml" : "typescript";
    return {
      project,
      source: {
        mode,
        configPath: DEFAULT_AGENT_CONFIG,
        ...mode === "extends-openpond-yaml" ? { extendsManifest: project.extendsManifest ?? OPENPOND_MANIFEST } : {}
      }
    };
  }
...
async function loadTypescriptProject(configPath) {
...
async function loadOpenPondYamlProject(configPath) {
  const parsed = (0, import_yaml.parse)(await readFile(configPath, "utf8"));
  if (!isRecord(parsed)) {
  ...
  return {
    ...
    resources: isRecord(parsed.resources) ? parsed.resources : void 0,
...
function yamlActions(manifest) {
  const manifestActions = Array.isArray(manifest.actions) ? manifest.actions : [];
  const actions = manifestActions.filter(isRecord).map((entry) => actionFromYaml(entry));
  if (actions.length > 0) return actions;
  const start = isRecord(manifest.start) ? manifest.start : {};
  return [actionFromYaml({ ...start, name: "chat" })];
}
function actionFromYaml(entry) {
  const name = stringValue(entry.name, "chat");
  const artifactPaths = Array.isArray(entry.artifactPaths) ? entry.artifactPaths.filter((item) => typeof item === "string") : [];
  return {
    name,
...
function commandWorkflow(action) {
  return {
    kind: "workflow",
  ...
  return volumes.filter(isRecord).map((entry) => ({
    ...
    provisioning: {
      ...
      create: {
        ...
        retention: booleanValue(entry.deleteOnSandboxDelete) === false ? "retain" : "delete-with-sandbox"
      }
    },
    state: { engine: "filesystem" }
  }));
}
function yamlSchedules(manifest) {
  ...
  return schedules.filter(isRecord).map((entry) => ({
    ...
    input: isRecord(entry.metadata) && isRecord(entry.metadata.input) ? entry.metadata.input : void 0,
    cron: optionalString(entry.cron),
    rate: optionalString(entry.rate),
    timezone: optionalString(entry.timezone)
  }));
}
function yamlIntegrations(manifest) {
  const integrations = [];
  const requiredLeases = isRecord(manifest.integrations) && Array.isArray(manifest.integrations.requiredLeases) ? manifest.integrations.requiredLeases : [];
...
function yamlEnv(manifest) {
  const env = isRecord(manifest.inputs) && Array.isArray(manifest.inputs.env) ? manifest.inputs.env : [];
...
function createArtifactIndex(project, artifactDir, options = {}) {
  const entries = [
    ...options.includeStandard === false ? [] : standardEntries(artifactDir, project.manifestMode !== "openpond-yaml"),
...
async function writeArtifactIndex(cwd, project, artifactDir, options = {}) {
  ...
  await writeJson(cwd, path4.join(artifactDir, "artifact-index.json"), index);
  return index;
}
async function assertArtifactSchemaCompatibility(cwd, index) {
  for (const entry of index.entries) {
    if (entry.format === "json") {
      const payload = JSON.parse(await readFile2(path4.resolve(cwd, entry.path), "utf8"));
      assertSchema(entry, payload.schema);
      continue;
    }
    if (entry.format === "jsonl") {
      const contents = await readFile2(path4.resolve(cwd, entry.path), "utf8");
    ...
    if (entry.format === "yaml") {
      const payload = (0, import_yaml2.parse)(await readFile2(path4.resolve(cwd, entry.path), "utf8"));
      assertSchema(entry, payload?.schema);
      continue;
    }
...
function evalResultsEntry(artifactDir) {
  return {
    path: path4.join(artifactDir, "eval-results.json"),
    kind: "eval-results",
    schema: ARTIFACT_SCHEMAS.evalResults,
    format: "json"
  };
}
function traceEntry(traceArtifactRef) {
  return {
...
function standardEntries(artifactDir, includeRuntimeBundle) {
  return [
    {
      path: path4.join(artifactDir, "artifact-index.json"),
      kind: "artifact-index",
      schema: ARTIFACT_SCHEMAS.artifactIndex,
      format: "json"
    },
    {
      path: path4.join(artifactDir, "agent-manifest.json"),
      kind: "agent-manifest",
      schema: ARTIFACT_SCHEMAS.agentManifest,
      format: "json"
    },
    {
      path: path4.join(artifactDir, "action-registry.json"),
      kind: "action-registry",
      schema: ARTIFACT_SCHEMAS.actionRegistry,
      format: "json"
    },
    {
      path: path4.join(artifactDir, "agent-inspect.json"),
      kind: "inspect",
      schema: ARTIFACT_SCHEMAS.inspect,
      format: "json"
    },
    {
      path: path4.join(artifactDir, "openpond-manifest.preview.yaml"),
      kind: "runtime-manifest-preview",
      schema: ARTIFACT_SCHEMAS.runtimeManifest,
      format: "yaml"
    },
    {
      path: path4.join(artifactDir, "runtime-bridge.mjs"),
      kind: "runtime-bridge",
      schema: ARTIFACT_SCHEMAS.runtimeBridge,
      format: "javascript"
    },
    ...includeRuntimeBundle ? [{
      path: path4.join(artifactDir, "runtime-bundle.mjs"),
      kind: "runtime-bundle",
      schema: ARTIFACT_SCHEMAS.runtimeBundle,
      format: "javascript"
    }] : [],
    {
      path: path4.join(artifactDir, "validator-report.md"),
...
function promptEntries(promptArtifacts) {
...
async function readExistingIndex(cwd, artifactDir) {
  const indexPath = path4.resolve(cwd, artifactDir, "artifact-index.json");
  if (!pathExists(indexPath)) return null;
  const parsed = JSON.parse(await readFile2(indexPath, "utf8"));
  return parsed.schema === ARTIFACT_SCHEMAS.artifactIndex ? parsed : null;
}
function existingEntriesThatStillExist(cwd, index) {
  return index.entries.filter((entry) => pathExists(path4.resolve(cwd, entry.path)));
}
function assertSchema(entry, actual) {
...
function capability(id, label, description, access2, leaseable) {
  return { id, label, description, access: access2, leaseable };
}
...
function createInspect(project, cwd, artifactDir = ARTIFACT_DIR) {
  return {
    ...
    runtimeManifest: path5.join(ARTIFACT_DIR, "openpond-manifest.preview.yaml"),
...
function createRuntimeManifest(project) {
  ...
  const validationCommands = project.validation?.commands && project.validation.commands.length > 0 ? project.validation.commands : ["openpond-agent validate"];
  return {
    schemaVersion: SDK_SCHEMA_VERSION,
    schema: ARTIFACT_SCHEMAS.runtimeManifest,
    name: project.name,
    ...
    schedules: (project.schedules ?? []).map((agentSchedule) => ({
      ...
      ...agentSchedule.scheduleType === "cron" ? { cron: agentSchedule.cron ?? agentSchedule.scheduleExpression ?? "" } : { rate: agentSchedule.rate ?? agentSchedule.scheduleExpression ?? "" },
    ...
    volumes: (project.volumes ?? []).map((agentVolume) => ({
      ...
      ...agentVolume.deleteOnSandboxDelete !== void 0 ? { deleteOnSandboxDelete: agentVolume.deleteOnSandboxDelete } : {}
    })),
...
function runtimeIntegrations(integrations = []) {
  return {
    requiredLeases: integrations.filter(
      (integration) => integration.required === true && isConnectedIntegrationProvider(integration.provider)
    ).map((integration) => ({
      provider: integration.provider,
      scopes: integration.scopes ?? [],
      capabilities: integration.capabilities ?? []
...
function runtimePermissions(integrations = []) {
...
function createActionRegistry(project) {
  return {
    schemaVersion: SDK_SCHEMA_VERSION,
    schema: ARTIFACT_SCHEMAS.actionRegistry,
...
function createRuntimeBridge(actionRegistry) {
...
function generatedArtifacts(artifactDir = ARTIFACT_DIR) {
  return {
    inspectJson: path5.join(artifactDir, "agent-inspect.json"),
    agentManifestJson: path5.join(artifactDir, "agent-manifest.json"),
    actionRegistryJson: path5.join(artifactDir, "action-registry.json"),
    runtimeManifestPreviewYaml: path5.join(artifactDir, "openpond-manifest.preview.yaml"),
    validatorReport: path5.join(artifactDir, "validator-report.md"),
    evalResultsJson: path5.join(artifactDir, "eval-results.json"),
...
function optionalPath(cwd, relativePath) {
  return pathExists(path5.join(cwd, relativePath)) ? relativePath : null;
}
...
function formatSkillMarkdown(input) {
...
// src/core/prompts.ts
var MAX_MARKDOWN_CHARS = 2e5;
var MAX_SKILL_FILES = 50;
async function compilePromptArtifacts(project, cwd, artifactDir) {
  return {
...
async function compileInstructions(instructions, cwd, artifactDir) {
  if (!instructions) return null;
  if (typeof instructions === "string") {
    return compileMarkdownSource({
      source: instructions,
      cwd,
      artifactRef: path6.join(artifactDir, "prompts", "instructions.md")
    });
  }
  return compileMarkdownSource({
...
async function compileSkill(skill, cwd, artifactDir) {
  const skillDir = path6.join(artifactDir, "skills", safePathSegment(skill.name));
  ...
  const artifactRef = path6.join(skillDir, "SKILL.md");
  ...
  for (const [relativePath, source] of fileEntries) {
    ...
    const fileArtifactRef = path6.join(skillDir, relativePath);
...
async function resolveMarkdownSource(source, cwd) {
  if (!source) return "";
  if (typeof source === "function") return source();
  if (source.startsWith("./")) return readFile3(path6.join(cwd, source.slice(2)), "utf8");
  return source;
}
...
function assertSafeRelativePath(relativePath, label) {
  if (relativePath.length === 0 || path6.isAbsolute(relativePath) || relativePath.split(/[\\/]+/).includes("..")) {
    throw new Error(`${label} path must stay inside the generated skill package: ${relativePath}`);
  }
}
function assertBoundedMarkdown(value, label) {
  if (value.length > MAX_MARKDOWN_CHARS) {
    throw new Error(`${label} markdown exceeds the limit of ${MAX_MARKDOWN_CHARS} characters.`);
  }
}

// src/core/runtime-bundle.ts
import path7 from "node:path";
import { build } from "esbuild";
async function writeRuntimeBundle(cwd, artifactDir) {
  const relativeOutputPath = path7.join(artifactDir, "runtime-bundle.mjs");
  await build({
    stdin: {
    ...
    outfile: path7.resolve(cwd, relativeOutputPath),
    bundle: true,
    platform: "node",
    target: "node22",
    format: "esm",
...
function formatYamlItem(value, indent) {
  if (isRecord(value)) {
    const entries = Object.entries(value);
    if (entries.length === 0) return "{}";
    const [first, ...rest] = entries;
    const [firstKey, firstValue] = first;
    const firstLine = isRecord(firstValue) || Array.isArray(firstValue) ? `${firstKey}:
${toYaml(firstValue, indent + 2)}` : `${firstKey}: ${yamlScalar(firstValue)}`;
...
function validateSourceOfTruth(project, cwd, issues) {
  const hasTypescriptConfig = pathExists(path8.join(cwd, DEFAULT_AGENT_CONFIG));
  const openPondManifestPath = path8.join(cwd, OPENPOND_MANIFEST);
  ...
  if (project.manifestMode === "extends-openpond-yaml") {
    ...
    if (!pathExists(path8.join(cwd, extendsManifest))) addIssue(issues, {
...
async function writeValidationReport(cwd, validation, artifactDir = ARTIFACT_DIR) {
  await writeText(cwd, path8.join(artifactDir, "validator-report.md"), formatValidationReport(validation));
}
...
function validateSourcePath(cwd, source, label, issuePath, issues) {
  ...
  if (!pathExists(path8.join(cwd, relativePath))) addIssue(issues, {
...
function isSafeRelativePath(relativePath) {
  return relativePath.length > 0 && !path8.isAbsolute(relativePath) && !relativePath.split(/[\\/]+/).includes("..");
}
...
async function buildCommand(options) {
  ...
  await writeJson(options.cwd, path9.join(options.outDir, "agent-manifest.json"), manifest);
  await writeJson(options.cwd, path9.join(options.outDir, "action-registry.json"), actionRegistry);
  await writeJson(options.cwd, path9.join(options.outDir, "agent-inspect.json"), inspect);
  await writeText(options.cwd, path9.join(options.outDir, "openpond-manifest.preview.yaml"), `${toYaml(createRuntimeManifest(project))}
`);
  await writeText(options.cwd, path9.join(options.outDir, "runtime-bridge.mjs"), createRuntimeBridge(actionRegistry));
...
async function writeTrace(cwd, name, state, artifactDir) {
  ...
  const relativePath = path10.join(traceDir(artifactDir), `${safeName}-${Date.now()}.jsonl`);
  const lines = [
    ...state.events.map((event) => traceEntry2("event", event)),
    ...state.artifacts.map((artifact) => traceEntry2("artifact", artifact)),
    ...state.commands.map((command) => traceEntry2("command", command))
  ].map((entry) => JSON.stringify(entry));
  await writeText(cwd, relativePath, `${lines.join("\n")}
`);
  return relativePath;
}
function traceEntry2(kind, value) {
  return {
    schemaVersion: SDK_SCHEMA_VERSION,
    schema: ARTIFACT_SCHEMAS.trace,
    kind,
...
async function evalCommand(options) {
  ...
  await writeJson(options.cwd, path11.join(options.outDir, "eval-results.json"), payload);
  const artifactIndex = await writeArtifactIndex(options.cwd, project, options.outDir, {
  ...
  await assertArtifactSchemaCompatibility(options.cwd, {
    ...artifactIndex,
    entries: artifactIndex.entries.filter(
      (entry) => entry.kind === "eval-results" || entry.kind === "trace-jsonl"
    )
  });
  if (options.json) console.log(JSON.stringify(payload, null, 2));
  else console.log(`${passed}/${results.length} evals passed
Wrote ${path11.join(options.outDir, "eval-results.json")}`);
  if (passed !== results.length) process.exitCode = 1;
}
async function resultPayload(cwd, artifactDir, evaluation, state, startedAt, status, error) {
...
async function sourceMetadata(cwd) {
  const configPath = path11.join(cwd, DEFAULT_AGENT_CONFIG);
  return {
    configPath: DEFAULT_AGENT_CONFIG,
    configHash: await fileHash(configPath)
  };
}
async function fixtureMetadata(cwd, fixtures) {
  return Promise.all(fixtures.map(async (fixture) => {
    const relativePath = fixture.startsWith("./") ? fixture.slice(2) : fixture;
    const filePath = path11.join(cwd, relativePath);
    return {
      path: relativePath,
...
async function fileHash(filePath) {
  const contents = await readFile4(filePath);
  return crypto3.createHash("sha256").update(contents).digest("hex");
}
...
async function inspectCommand(options) {
  const project = await loadAgentProject(options.cwd);
  const inspect = createInspect(project, options.cwd, options.outDir);
  await writeJson(options.cwd, path12.join(options.outDir, "agent-inspect.json"), inspect);
  if (options.json) {
    console.log(JSON.stringify(inspect, null, 2));
    return;
  }
  console.log(`Wrote ${path12.join(options.outDir, "agent-inspect.json")}`);
}

// src/commands/init.ts
import { cp, mkdir as mkdir2, readFile as readFile5, readdir, writeFile as writeFile2 } from "node:fs/promises";
import path13 from "node:path";
import { fileURLToPath } from "node:url";
var TEMPLATE_NAMES = ["blank-agent", "customer-reply-agent", "integration-heavy-agent"];
async function initCommand(options) {
  ...
  const templateDir = path13.join(sdkRoot, "templates", templateName);
...
async function assertWritableTarget(cwd, force) {
...
function packageRoot() {
  const distRoot = path13.dirname(fileURLToPath(import.meta.url));
  return path13.basename(distRoot) === "dist" ? path13.dirname(distRoot) : path13.resolve(distRoot, "..", "..");
}
async function rewriteLocalSdkDependency(cwd, sdkRoot) {
...
async function tracesCommand(options) {
  const tracesDir = traceDir(options.outDir);
  try {
    const traces = (await readdir2(path14.join(options.cwd, tracesDir))).filter((entry) => entry.endsWith(".jsonl")).sort();
    if (options.json) {
      console.log(JSON.stringify({ traces: traces.map((trace) => path14.join(tracesDir, trace)) }, null, 2));
      return;
    }
    if (traces.length === 0) console.log("No trace artifacts.");
    else for (const trace of traces) console.log(path14.join(tracesDir, trace));
  } catch {
...
async function parseArgs(args) {
  ...
  let json = false;
  let actionName = command === "run" ? maybeActionName : void 0;
  let templateName = command === "init" ? maybeActionName : void 0;
  let force = false;
  let input;
  ...
  for (let index = 0; index < optionArgs.length; index += 1) {
    ...
    if (arg === "--input-file") {
      ...
      input = JSON.parse(await readFile6(path15.resolve(cwd, value), "utf8"));
      index += 1;
      continue;
    }
  ...
  return {
    ...
    templateName,
    force,
    input: input ?? (command === "run" ? parseActionInputEnv(process.env) : void 0)
  };
}
