...
async function writeArtifactIndex(cwd, project, artifactDir, options = {}) {
  ...
  await writeJson(cwd, path3.join(artifactDir, "artifact-index.json"), index);
  return index;
}
async function assertArtifactSchemaCompatibility(cwd, index) {
  for (const entry of index.entries) {
    if (entry.format === "json") {
      const payload = JSON.parse(await readFile(path3.resolve(cwd, entry.path), "utf8"));
      assertSchema(entry, payload.schema);
      continue;
    }
    if (entry.format === "jsonl") {
      const contents = await readFile(path3.resolve(cwd, entry.path), "utf8");
    ...
    if (entry.format === "yaml") {
      const payload = (0, import_yaml.parse)(await readFile(path3.resolve(cwd, entry.path), "utf8"));
      assertSchema(entry, payload?.schema);
      continue;
    }
...
function evalResultsEntry(artifactDir) {
  return {
    path: path3.join(artifactDir, "eval-results.json"),
    kind: "eval-results",
...
function standardEntries(artifactDir, includeRuntimeBundle) {
  return [
    {
      path: path3.join(artifactDir, "artifact-index.json"),
      kind: "artifact-index",
      schema: ARTIFACT_SCHEMAS.artifactIndex,
      format: "json"
    },
    {
      path: path3.join(artifactDir, "agent-manifest.json"),
      kind: "agent-manifest",
      schema: ARTIFACT_SCHEMAS.agentManifest,
      format: "json"
    },
    {
      path: path3.join(artifactDir, "action-registry.json"),
      kind: "action-registry",
      schema: ARTIFACT_SCHEMAS.actionRegistry,
      format: "json"
    },
    {
      path: path3.join(artifactDir, "agent-inspect.json"),
      kind: "inspect",
      schema: ARTIFACT_SCHEMAS.inspect,
      format: "json"
    },
    {
      path: path3.join(artifactDir, "openpond-manifest.preview.yaml"),
      kind: "runtime-manifest-preview",
      schema: ARTIFACT_SCHEMAS.runtimeManifest,
      format: "yaml"
    },
    {
      path: path3.join(artifactDir, "runtime-bridge.mjs"),
      kind: "runtime-bridge",
      schema: ARTIFACT_SCHEMAS.runtimeBridge,
      format: "javascript"
    },
    ...includeRuntimeBundle ? [{
      path: path3.join(artifactDir, "runtime-bundle.mjs"),
      kind: "runtime-bundle",
      schema: ARTIFACT_SCHEMAS.runtimeBundle,
      format: "javascript"
    }] : [],
    {
      path: path3.join(artifactDir, "validator-report.md"),
...
async function readExistingIndex(cwd, artifactDir) {
  const indexPath = path3.resolve(cwd, artifactDir, "artifact-index.json");
...
function existingEntriesThatStillExist(cwd, index) {
  return index.entries.filter((entry) => pathExists(path3.resolve(cwd, entry.path)));
}
